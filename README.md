...
- [Vue-slot新旧语法对比 (Vue-slot syntax comparison:New vs. Old)](#vue-slot新旧语法对比-vue-slot-syntax-comparisonnew-vs-old)
    - [前言](#前言)
    - [1-1.具名插槽(named slots) - 旧](#1-1具名插槽named-slots---旧)
        - [父组件中的代码如下:](#父组件中的代码如下)
        - [子组件的代码如下:](#子组件的代码如下)
        - [render效果如下:](#render效果如下)
    - [1-2.默认插槽(default slots) - 旧](#1-2默认插槽default-slots---旧)
        - [父组件代码与1-1相同:](#父组件代码与1-1相同)
        - [子组件代码如下:](#子组件代码如下)
        - [render效果如下:](#render效果如下-1)
    - [2-1.具名插槽(named slots) - 新](#2-1具名插槽named-slots---新)
        - [父组件代码如下:](#父组件代码如下)
        - [子组件代码如下:](#子组件代码如下-1)
        - [render效果如下:](#render效果如下-2)
    - [2-2.默认插槽(default slots)-新](#2-2默认插槽default-slots-新)
        - [父组件中代码如下:](#父组件中代码如下)
        - [子组件中代码如下:](#子组件中代码如下)
    - [3.后备内容(Fallback Content)](#3后备内容fallback-content)
        - [父组件代码如下:](#父组件代码如下-1)
        - [子组件代码如下:](#子组件代码如下-2)
        - [render效果如下:](#render效果如下-3)
        - [当组件内添加内容替换后备内容:](#当组件内添加内容替换后备内容)
        - [render效果如下:](#render效果如下-4)
    - [4.编译作用域(compilation scope)](#4编译作用域compilation-scope)
        - [父组件代码如下:](#父组件代码如下-2)
        - [子组件代码如下:](#子组件代码如下-3)
        - [render效果如下:](#render效果如下-5)
        - [当组件内添加内容用{{user.lastName}}替换后备内容:](#当组件内添加内容用userlastname替换后备内容)
        - [render效果如下:](#render效果如下-6)
    - [5-1.作用域插槽(scoped slots) - 旧](#5-1作用域插槽scoped-slots---旧)
        - [render效果如下:](#render效果如下-7)
    - [5-2.作用域插槽(scoped slots) -新](#5-2作用域插槽scoped-slots--新)
        - [父组件中代码如下:](#父组件中代码如下-1)
        - [报错:](#报错)
        - [子组件模板中的代码如下:](#子组件模板中的代码如下)
        - [render效果如下:](#render效果如下-8)
    - [6.动态插槽名(Dynamic Slot Names)](#6动态插槽名dynamic-slot-names)
        - [示例:](#示例)
    - [7.具名/默认插槽的嵌套和混合用法](#7具名默认插槽的嵌套和混合用法)
  - [参考:](#参考)
